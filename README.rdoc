support for redmine 3.x(not finished yet)

how to install
1 copy to plugins
2 bundle update
3 bundle exec rake redmine:plugins:migrate NAME=plugin_name VERSION=0 RAILS_ENV=production

change:
dispatcher --> ActionDispatch::Callbacks.to_prepare do
named_scope--> scope
ActionController::Routing::Routes.draw do-->RedmineApp::Application.routes.draw do
